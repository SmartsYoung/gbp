...
FOREND:
	for {
		...
		case "2":
			file, err := os.Open("/src/chapter01/io/01.txt") //func Open(name string) (*File, error) { return OpenFile(name, O_RDONLY, 0)}
			// Open函数返回的是 File 类型， 而 File 实现了 io.Reader接口， 因此 ReadFrom 传入参数 file 是可行的
			if err != nil {
